...
class TestApi(unittest.TestCase):
    ...
    def test_fielding_valid(self):
        response = requests.get(base_url.format("fieldingStats", VALID_API_KEY, 2009, 9032, 0))
        self.assertEqual(response.status_code,200)
        self.assertGreaterEqual(len(response.json()),0)

    def test_bowling_valid(self):
        response = requests.get(base_url.format("bowlingStats", VALID_API_KEY, 2009, 9032, 0))
        self.assertEqual(response.status_code,200)
        self.assertGreaterEqual(len(response.json()),0)

    def test_batting_valid(self):
        response = requests.get(base_url.format("battingStats", VALID_API_KEY, 2009, 9032, 0))
        self.assertEqual(response.status_code,200)
        self.assertGreaterEqual(len(response.json()),0)

    def test_year_outside_range(self):
        response = requests.get(base_url.format("bowlingStats", VALID_API_KEY, 2000, 9032, 0))
        self.assertEqual(response.status_code, 404)

    def test_invalid_finals_flag(self):
        response = requests.get(base_url.format("bowlingStats", VALID_API_KEY, 2000, 9032, 12))
        self.assertEqual(response.status_code, 404)

    def test_missing_argument(self):
        url = base_url.format("bowlingStats", VALID_API_KEY, 2000, 9032, 12)
        url = url.replace("&finals_flag=12","")
        response = requests.get(url)
        self.assertEqual(response.status_code, 400)

    def test_bad_method_call(self):
        response = requests.get(base_url.format("someRandomMethod", VALID_API_KEY, 2000, 9032, 12))
        self.assertEqual(response.status_code, 404)